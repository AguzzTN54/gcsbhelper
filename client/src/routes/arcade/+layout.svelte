...
<div class="flex items-end justify-center size-full">
	<div
		class="w-[95%] md:w-9/12 bg-gray-100 h-[calc(90%-2rem)] sm:h-[90%] relative rounded-t-3xl rounded-tr-none !border-b-0 brutal-border brutal-shadow"
	>
		<!-- Countdown -->
		<div
			class="absolute top-0 left-0 z-10 -translate-y-[calc(100%+2.5rem)] sm:-translate-y-full flex justify-center sm:justify-start w-full sm:w-fit"
		>
			<Countdown />
		</div>
		...
		<!-- Menu -->
		<div
			class="absolute rounded-tr-3xl bottom-full right-0 translate-y-[1.2rem] pb-3 brutal-shadow brutal-border translate-x-[6px] overflow-hidden"
		>
			<NavMenu />
		</div>
