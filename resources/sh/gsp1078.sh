...
########################################
YAML_PATTERN="_REGION: ([a-z]+)-([a-z0-9]+)"
YAML_REPLACE="_REGION: $REGION\n\noptions:\n  logging: CLOUD_LOGGING_ONLY"

sed -ri "s/$YAML_PATTERN/$YAML_REPLACE/" branch-cloudbuild.yaml
sed -ri "s/$YAML_PATTERN/$YAML_REPLACE/" master-cloudbuild.yaml
sed -ri "s/$YAML_PATTERN/$YAML_REPLACE/" tag-cloudbuild.yaml

TRIGGER_REPLACE="\"serviceAccount\":\"projects\/$PROJECT_ID\/serviceAccounts\/$PROJECT_NUMBER-compute@developer.gserviceaccount.com\",\n  \"filename"
sed -ri "s/\"filename/$TRIGGER_REPLACE/" branch-trigger.json
sed -ri "s/\"filename/$TRIGGER_REPLACE/" master-trigger.json
sed -ri "s/\"filename/$TRIGGER_REPLACE/" tag-trigger.json
